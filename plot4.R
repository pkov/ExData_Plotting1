...
par(mfcol=c(2,2))
plot(with(p,strptime(paste(Date,Time),format="%Y-%m-%d %H:%M:%S")),p$Global_active_power,type="l",xlab="",ylab="Global Active Power")
...
plot(with(p,strptime(paste(Date,Time),format="%Y-%m-%d %H:%M:%S")),p$Voltage,type="l",xlab="datetime",ylab="Voltage")
plot(with(p,strptime(paste(Date,Time),format="%Y-%m-%d %H:%M:%S")),p$Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power")
dev.off() 
